function [ElementMat2DK, ElementMat2DM] = ElementMatrices2D(Nodes, p, MatProp, physics)
    ...
    % z4=Nodes(4,3);
    %
    % Auslesen der Integrationsstuetzstellen und Gewichte
    [IntNodes, IntWeights] = NumIntTabFEM(p);
    % Auslesen der Materialeigenschaften aus der "MatProp"-Matrix
    Emod = MatProp(1);
    nue = MatProp(2);
    rho = MatProp(3);
    thick = MatProp(4);

    if physics == "PlaneStrain"
        D = Emod / (1 + nue) / (1 - 2 * nue) * [1 - nue nue 0; nue 1 - nue 0; 0 0 1/2 - nue];
    elseif physics == "PlaneStress"
        D = Emod / (1 - nue^2) * [1 nue 0; nue 1 0; 0 0 (1 - nue) / 2];
    end

    % Preallokation der Steifigkeitsmatrix
    ElementMat2DK = zeros(2 * (p + 1)^2, 2 * (p + 1)^2);
    % Preallokation der Massenmatrix
    ElementMat2DM = zeros(2 * (p + 1)^2, 2 * (p + 1)^2);
    % Preallokation der B-Matrix (enthaelt Ableitungen der Formfunktionen)
    GradMat = zeros(3, (p + 1)^2 * 2);
    % Preallokation der N-Matrix (enthaelt die Formfunktionen)
    NMat = zeros(2, (p + 1)^2 * 2);
    % Schleife von i=1 bis Anzahl der Knoten pro Element
    for i = 1:(p + 1)^2
