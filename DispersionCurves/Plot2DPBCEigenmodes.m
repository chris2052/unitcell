...
%     title([num2str(numEig),'. Eigenmode, f=',num2str(f,'%.2f'),' [Hz] '])
    if K(1)==0 && K(2)==0
    title([num2str(numEig,'%.0f') '. Eigenmode, $f$ = ' num2str(abs(f),'%.1f') ' [Hz], point $\Gamma$'] ,'interpreter','latex');
    end
    if K(1)==pi() && K(2)==0
    title([num2str(numEig,'%.0f') '. Eigenmode, $f$ = ' num2str(abs(f),'%.1f') ' [Hz], point $X$'] ,'interpreter','latex');
    end
    if K(1)==pi() && K(2)==pi()
    title([num2str(numEig,'%.0f') '. Eigenmode, $f$ = ' num2str(abs(f),'%.1f') ' [Hz], point $M$'] ,'interpreter','latex');
    end
    figureHandle4 = gcf;
