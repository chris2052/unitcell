...
freq = 2000;   % variable frequency range

fprintf('calculation set for: \n\n')
fprintf('       diameter = NaN \n')
fprintf('        angular = NaN deg \n')
fprintf('       distance = NaN \n')
...
idx_displ_bound = mphselectbox(model, 'geom1', [0.5*length+0.3*w_bound 0.5*length+0.7*w_bound; -0.5*length-1.3*w_bound 0.5*length+1.3*w_bound], 'boundary');
...
model.component('comp1').mesh('mesh1').feature('ftri3').feature('size1').set('hmax', 10);
...
figurenamegraph = [num2str(d), '_graph_norot_', num2str(y_min), '.fig'];
...
figurenamesurf = [num2str(d), '_surf_norot_', num2str(min_freq), '.fig'];
