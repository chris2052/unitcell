tic
% every iteratin outer border less
for border = 14
...
ang_deg = 45;        % variable
dist = 0.5*d;      % variable
freq = 27000;   % variable frequency range
...
%% create rotation
    ...
    for f = 1:10
        ...
        count = count - rem(count,1)-border;
        if count > 1
    ...
    figurenamegraph = [num2str(border), '_border_graph_', num2str(y_min), '.fig'];
    ...
    figurenamesurf = [num2str(border), '_border_surf_', num2str(min_freq), '.fig'];
    ...
    %% save workspace
    workspacename = [num2str(border), '_border_', num2str(numel(idx_holes_dom))];
    save(workspacename);
