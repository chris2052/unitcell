...
%     ModelUtil.showProgress(true);
    ...
    d = 16;     % variable
    dist = 0.5*d;      % variable
    freq = 29300;   % variable frequency range      29300
   ...
   % fprintf('       with step: %d mHz \n', freq(2)-freq(1))
    ...
    model.param.set('E2', '200e9[Pa]', 'outer mat2');        % concrete
    ...
    %% create rotation
    count = length/dist_c;
    count = count-rem(count,1)-1 - border;
    ...
    % create box selection node for items inside zone (after delete them)
    ...
    figurenamegraph = [num2str(border*0.5), '_border_graph_', num2str(y_min), '.fig'];
    ...
    figurenamesurf = [num2str(border*0.5), '_border_surf_', num2str(min_freq), '.fig'];
    ...
    %% save workspace
    try
    workspacename = [num2str(border*0.5), '_border_', num2str(numel(idx_holes_dom))];
    save(workspacename);
    catch
        disp('blablub');    
    end
