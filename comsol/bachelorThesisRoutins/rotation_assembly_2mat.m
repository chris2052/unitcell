...
%     ModelUtil.showProgress(true);
    ...
    fprintf('       d_out = %d m \n', d_out)
    fprintf('        d_in = %d m \n', d_in)
    fprintf('   thickness = %d m \n', t)
    fprintf('        angular = %d deg \n', ang_deg)
    fprintf('       distance = %d m \n', dist)
    fprintf(' frequency range: %d to %d \n', freq(1), freq(end))
    fprintf('       with step: %d \n', freq(2)-freq(1))

    %% model parameter
    model.param.set('d', d_out);
    model.param.set('d_in', d_in);
    model.param.set('r', r_out);
    model.param.set('r_in', r_in);
    ...
    model.param.set('rho1', '1000[kg/m^3]', 'Boden density');
    model.param.set('nu1', '0.3', 'Boden poissons ratio');
    model.param.set('E2', '0.01e9[Pa]', 'outer mat2');        % outer mat
    model.param.set('rho2', '900[kg/m^3]', 'outer mat2');
    model.param.set('nu2', '0.49', 'outer mat2');
    model.param.set('E3', '200e9[Pa]', 'inner mat3');      % inner mat
    model.param.set('rho3', '7850[kg/m^3]', 'inner mat3');
    model.param.set('nu3', '0.3', 'inner mat3');
    ...
    for m=0:ang:(2*pi-ang)
        for n=0:count
            ...
            model.geom('geom1').feature(tag).set('r', r_out);
    ...
    for m=0:ang:(2*pi-ang)
        for n=0:count
            tag = model.geom('geom1').feature.uniquetag('c');
            model.geom('geom1').feature.create(tag, 'Circle');
            model.geom('geom1').feature(tag).set('r', r_in);
            model.geom('geom1').feature(tag).set('pos', [(dist_0+dist_c*n)*cos(m) (dist_0+dist_c*n)*sin(m)]);
        end
    end
    ...
    figurenamegraph = [num2str(d_out), '_graph_', num2str(y_min), '.fig'];
    ...
    figurenamesurf = [num2str(d_out), '_surf_', num2str(min_freq), '.fig'];
    ...
    %% save workspace
    workspacename = [num2str(d_out), '_', num2str(numel(idx_holes_dom))];
    save(workspacename);
