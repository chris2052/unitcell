...
%     d = 16;     % variable
    dist = d;      % variable
    freq = 28900;   % variable frequency range
...
%     fprintf('       with step: %d mHz \n', freq(2)-freq(1))
    ...
    model.param.set('E2', '30e9[Pa]', 'outer mat2');        % conc
    ...
    figurenamegraph = ['dd_', num2str(d), '_rect_conc', num2str(y_min), '.fig'];
    ...
    figurenamesurf = ['d05d_', num2str(d), '_surf_rect_conc', num2str(min_freq), '.fig'];
    ...
    %% save workspace
    try
    workspacename = ['dd_', num2str(d), '_rect_conc', num2str(numel(idx_holes_dom))];
    save(workspacename);
    catch
        disp('blablub');    
    end
